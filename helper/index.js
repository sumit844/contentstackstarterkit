...
export const getFiremanRes = async (entryUrl) => {
    const response = await Stack.getEntryByUrl({
        contentTypeUid: "character_detail_page",
        entryUrl,
        jsonRtePath: ["description"],
    });
    liveEdit && addEditableTags(response[0], "character_detail_page", true);
    return response[0];
};

export const getCharacterGallaryRes = async (entryUrl) => {
    const response = await Stack.getEntryByUrl({
        contentTypeUid: "charact_gallary_page",
        entryUrl,
        referenceFieldPath:['character_heros'],
        jsonRtePath:['character_heros.description']
    });
    liveEdit && addEditableTags(response[0], "charact_gallary_page", true);
    return response[0];
};
